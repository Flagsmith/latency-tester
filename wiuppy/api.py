...
class WIU:
    ...
    def __init__(self, client, token):
        ...
        """
        self.headers["Auth"] = (
            "Bearer " + self._is_valid_id(client) + " " + self._is_valid_id(token)
        )
    ...
    def submit(self, target, servers, tests, options={}):
        ...
        """
        data = {"uri": target, "tests": tests, "sources": servers, "options": options}
    ...
    def _post(self, endpoint, data):
        try:
            return requests.post(
                self.URL + endpoint, headers=self.headers, data=json.dumps(data)
            ).json()
        except requests.exceptions.RequestException as e:
            raise Error("Error talking to the API: " + str(e))
