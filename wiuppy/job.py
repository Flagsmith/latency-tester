...
class Job:
    ...
    Attributes:
        ...
        servers:   [list]   WonderNetwork servers to perform tests from
    ...
    def submit(self):
        ...
        Returns:
            self
        """
        self.id = self._api.submit(self.uri, self.servers, self.tests, self.options)
    ...
    def __str__(self):
        ...
        if self.results:
            out["results"] = {
                server: {test: results["summary"] for (test, results) in tests.items()}
